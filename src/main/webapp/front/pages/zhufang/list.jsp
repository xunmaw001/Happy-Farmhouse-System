                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhufangName" id="zhufangName" placeholder="住房编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="zhufangTypes" id="zhufangTypes" lay-filter="zhufangTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zhufangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="leixingTypes" id="leixingTypes" lay-filter="leixingTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in leixingTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../zhufang/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.zhufangPhoto?item.zhufangPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../zhufang/detail.jsp?id='+item.id)"><span>{{item.zhufangName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.zhufangNewMoney"><span style="color: red;">{{item.zhufangNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zhufangContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zhufangContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zhufangName').val() != null && jquery('#zhufangName').val() != "" && jquery('#zhufangName').val() != "null" ) {
                param['zhufangName'] = jquery('#zhufangName').val();
            }
         
            var zhufangTypes = $("#zhufangTypes").find("option:selected").val();
            if (zhufangTypes != null && zhufangTypes != "" && zhufangTypes != "null") {
                param['zhufangTypes'] = zhufangTypes;
            }
     
            var leixingTypes = $("#leixingTypes").find("option:selected").val();
            if (leixingTypes != null && leixingTypes != "" && leixingTypes != "null") {
                param['leixingTypes'] = leixingTypes;
            }
                        // 获取列表数据
            http.request('zhufang/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function zhufangTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhufang_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.zhufangTypesList = res.data.list;
               }
           });
       }
       ...
       function leixingTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=leixing_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.leixingTypesList = res.data.list;
               }
           });
       }
